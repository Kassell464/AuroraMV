...
from core.renderer.shader import create_program
...
class Renderer:
    ...
    def update(self, time: float, audio_state: object | None = None) -> None:
        """更新帧状态。

        audio_state 参数为音频响应系统预留（阶段 3 接入），当前未使用。
        """
        self._time = time
        self.camera.update(time)

    def render(self) -> None:
        """渲染一帧：清屏（背景颜色）+ 绘制带纹理的四边形（简单动画）。"""
